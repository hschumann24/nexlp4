...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (cookbookButton) {
        cookbookButton.addEventListener("click", () => {
            const customizationDetails = JSON.parse(localStorage.getItem("customizationDetails") || "{}");
...
function loadCart() {
    ...
    cart.forEach((item, index) => {
        ...
        const productNumber = item.productNumber || "Unknown";
        const price = item.price ? item.price.toFixed(2) : "N/A"; // Use fallback if price is missing

        cartHTML += `
            <div class="cart-item">
                <strong>${productName}</strong> (Product Number: ${productNumber})<br>
                Price: $${price}<br>
                <em>Customizations:</em> ${Object.entries(item)
...
// Add this to your document's on-load handler
document.addEventListener("DOMContentLoaded", () => {
